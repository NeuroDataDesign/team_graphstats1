...
class Coherence(BaseSubgraph):
    ...
    coherent estimator, which is constrained by the number of edges and by
    the number of vertices that the edges in the ss may be incident to.
    ...
    def __init__(self, constraints):
        if (
            not isinstance(constraints, (int))
            or not isinstance(constraints, (np.ndarray))
            or len(constraints) != 2
        ):
            msg = "Constraints must be either an int or a vector of length 2."
            raise TypeError(msg)
    ...
    def fit(self, X, y):
        ...
        Parameters
        ----------
        X : np.ndarray, shape (n_vertices, n_vertices, n_graphs)
        ...
        # coherent
            ...
            nedges = self.constraints[0]
            nverts = self.constraints[1]

...
class Screen(BaseSubgraph):
    ...
    higher than the threshold is included as the signal subgraph (ss).
    ...
    def __init__(self, stat, corr_thresh):
        ...
        if not isinstance(corr_thresh, numbers.Real):
            msg = "corr_thresh must be float or int"
            raise ValueError(msg)

        if corr_thresh < 0 or corr_thresh > 1:
            msg = "corr_thresh must be in the interval [0,1]"
            raise ValueError(msg)
    ...
    def _fit_transform(self, X, y):
        ...
        subgraph_n_vert = len(self.subgraph_verts)
        S_hat = np.zeros((self.n_graphs, subgraph_n_vert, subgraph_n_vert))
        for i in range(self.n_graphs):
            S_hat[i] = X[i][self.subgraph_verts][:, self.subgraph_verts]

...
class Sparse(BaseSubgraph):
    ...
    -------
    object
        Containing the ss and the classifier:
            - active_nodes : np.ndarray, shape ``(n_vertices, 1)``
                Estimated ss.
            - b : float
                Intercept value
            - beta : np.ndarray
    ...
    References
    ----------
    .. [1] J. Relion, et al. "Network Classification with Applications to Brain
    Connectomics," arXiv: 1701.08140 [stat.ME], 2019
    """

    def __init__(self, lambda_=0, gamma=1e-5, rho=0):
        if not isinstance(lambda_, numbers.Real):
            msg = "lambda must be float or int"
            raise ValueError(msg)
        if lambda_ < 0:
            msg = "lambda must be a positive value"
            raise ValueError(msg)

        if not isinstance(gamma, numbers.Real):
            msg = "gamma must be float or int"
            raise ValueError(msg)

        if not isinstance(rho, numbers.Real):
            msg = "rho must be float or int"
            raise ValueError(msg)
        if rho < 0 or rho > 1:
            msg = "rho must be in the interval [0,1]"
            raise ValueError(msg)
    ...
    def _logistic_lasso(self, lambda1, lambda2):
        ...
        def b_hessian(Xbeta, b):
            exp1 = np.exp(-self.y * (Xbeta + b))
            exp2 = np.exp(self.y * (Xbeta + b))
            return np.sum(1 / (exp1 + exp2 + 2)) / n

        def grad_f(Xbeta, b, beta):
            prob = self.y / (1 + np.exp(self.y * (Xbeta + b)))
            return -_cross(self.x, prob) / n + self.gamma * beta
        ...
        def proximal(u, lambda_, beta_startprox=None, tol=1e-7):
            if lambda2 > 0:
                ...
                ret1 = np.sign(np.max(np.abs(u) - (lambda1 * lambda_), 0))
                return ret1, lambda1
            else:
                return u, lambda1
        ...
        optimal = self._fista(proximal, b_step, f, grad_f, penalty, beta_start, b_start)

        return optimal

    def _fista(self, proximal, b_step, f, grad_f, penalty, beta_start, b_start):
        TOLERANCE = 1e-6
        MAX_STEPS = 300
        MAX_TIME = 10800
        ...
        while (k <= 5) or (
            ...
            if newf < best_f:
                ...
                self.best_prox = prox
                self.best_f = newf
            fscale = np.absolute(crit_f - crit_f_1) / crit_f
            if (crit_f == 0) or (fscale < 0.1):
                tk *= 2
            crit_f_1 = crit_f
            beta_path.append(xk1)
    ...
    def fit(self, x, y, xtest, ytest):
        """
        Performs sparse optimization on graphs to estimate signal subgraph.

        Parameters
        ----------
        x : np.ndarray
            Matrix with the training samples, where each row represents the
            vectorized (by column order) upper triangular part of a network
            adjacency matrix.
        y : np.ndarray, shape ``(n_training_graphs, 1)``
            A vector containing the class labels of the training samples.
        xtest : np.ndarray
            Matrix containing the test samples, with each row representing an
            upper-triangular vectorized adjacency matrix.
        ytest : np.ndarray, ``(n_testing_graphs, 1)``
            A vector containing the class labels of the testing samples.
        ...
        if type(xtest) is not np.ndarray:
            msg = "xtest must be numpy.ndarray"
            raise TypeError(msg)
        if type(ytest) is not np.ndarray:
            msg = "ytest must be numpy.ndarray"
            raise TypeError(msg)

        if len(x.shape) != 2:
            msg = "x must be a matrix"
            raise ValueError(msg)
        if len(xtest.shape) != 2:
            msg = "xtest must be a matrix"
            raise ValueError(msg)
        if len(y.shape) != (2 or 1):
            msg = "y must be a vector"
            raise ValueError(msg)
        if len(ytest.shape) != (2 or 1):
            msg = "ytest must be a vector"
            raise ValueError(msg)

        if y.shape != (x.shape[0], 1):
            msg = "y must have shape (x.size, 1)"
            raise ValueError(msg)
        if ytest.shape != (xtest.shape[0], 1):
            msg = "ytest must have shape (xtest.size, 1)"
            raise ValueError(msg)

        # Infer n_vertices
        ind_val = x.shape[1]
        self.nodes = int((1 + np.sqrt(1 + 8 * ind_val)) / 2)
        ...
        # Estimated training labels and its error
        ...
        # Estimated testing labels and its error
...
def _construct_d(nodes=264):
    ...
    for i in range(0, nodes):
        for a in range(0, nodes - 1):
            ind = indexer[count] - 1
            D[(i * (nodes - 1)) : ((i + 1) * (nodes - 1))][a][ind] = 1
            count += 1

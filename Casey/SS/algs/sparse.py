    ...
    def _logistic_lasso(self, lambda1, lambda2):
        ...
        def b_hessian(Xbeta, b):
            return (
                np.sum(
                    1
                    / (np.exp(-self.y * (Xbeta + b)) + np.exp(self.y * (Xbeta + b)) + 2)
                )
                / n
            )
        ...
        def grad_f(Xbeta, b, beta):
            return (
                -_cross(self.x, self.y / (1 + np.exp(self.y * (Xbeta + b)))) / n
                + self.gamma * beta
            )
        ...
        def proximal(u, lambda_, beta_startprox=None, tol=1e-7):
            if lambda2 > 0:
                ...
                return (np.sign(np.max(np.abs(u) - (lambda1 * lambda_), 0)), lambda1)
            else:
                return u, lambda1
...
def _construct_d(nodes=264):
    ...
    for i in range(0, nodes):
        for a in range(0, nodes - 1):
            D[(i * (nodes - 1)) : ((i + 1) * (nodes - 1))][a][indexer[count] - 1] = 1
            count += 1

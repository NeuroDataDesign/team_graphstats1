...
class ItScreen(BaseSubgraph):
    ...
    def __init__(self, stat, delta, sg_n_verts):
        ...
        if not isinstance(sg_n_verts, int):
            raise ValueError("sg_n_verts must be an int")
    ...
    def fit(self, X, y):
        ...
        if self.sg_n_verts < 0 or self.sg_n_verts > self.n_vertices:
            msg = "sg_n_verts must be between 0 and n_vertices"
            raise ValueError(msg)

        # Create empty array to store correlation values
        cors = np.zeros((self.n_vertices, 1))
        sg_inds = np.arange(self.n_vertices, dtype="int64")
        itr = 1
        ...
        val = 1 - self.delta
        while (val ** itr) * self.n_vertices > self.sg_n_verts:

            # Creating new temporary set of subgraphs
            screen = NonItScreen(self.stat, tmpq)
